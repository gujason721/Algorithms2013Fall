...
void InorderTreewalk(struct rb_node_t * tree){
    if(tree){
...
int main()
{
    ...
    //search in the RB-tree && delete operation in the RB-tree

    key = rand() % count;
    if ((node = rb_search(key, root)))
    {
        printf("\nsearch key %d success! it is in the %i th node.\n", key, node->data);
        printf("\nNow we delete it:\n");
        if ((root = rb_erase(key, root)))
            ...
            {
                printf("Delete key %d error! \n", i, key);
            }
    }
    else
    {
        printf("search key %d error!\n", i, key);
            exit(-1);
    }
