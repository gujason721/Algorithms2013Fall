...
public class BTree {
	...
	private int deleteMax(BNode y){
		// TODO: do this and deleteMin in 1 pass instead of two
		int max = getMax(y);
		delete(max,y);
		return max;
	}
	
	private int getMax(BNode y){
		if (y.leaf) {
			return y.key[y.n - 1];
		} else {
			return getMax (y.c[y.n]);
		}
	}
	
	private int getMin(BNode y){
		if (y.leaf) {
			return y.key[0];
		} else {
			return getMin (y.c[0]);
			...
			if (subTreeContK.n == t-1) { // case 3a,b
				...
				if (lSibling != null && lSibling.n >= t) { // case 3a.1 - left sibling has at least t keys
					...
					if (rSibling != null){
						...
						idxOfSubTreeContK--; // since we just deleted the left sibling, this index has to decrement
					}
				}
			}
			delete(k,x.c[idxOfSubTreeContK]);
		}
	}
}
