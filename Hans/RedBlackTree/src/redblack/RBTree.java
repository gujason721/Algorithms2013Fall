...
public class RBTree {
	...
	public void rotateRight(RBNode y){
		...
		x.p = y.p;
		if (y.p == nil) // x may now be the new root
			root = x;
		else if (y == y.p.right) // if y was on its parent's right side 
			y.p.right = x;  // then x is the new right child of that parent
		else y.p.left = x; // otherwise x is the left child of that parent
	...
	public void rotateLeft(RBNode x){
		...
		y.p = x.p;
		if (x.p == nil) // y may now be the new root
			root = y;
		else if (x == x.p.left) 
			x.p.left = y;
		else x.p.right = y;
